...
namespace LCM.LCM
{
  ...
  public class UDPMulticastProvider : Provider
  {
    ...
    private const string DEFAULT_NETWORK = "239.255.76.67:7667";
		...
		// NOTE(milo): Changed this because DEFAULT_TTL = 0 was causing a crash!
		// Wasn't able to figure out the problem, but setting UdpClient.Ttl = 0
		// results in a System.ArgumentException being thrown.
		// Check ~/.config/unity3d/Editor.log if this problem shows up again.
    private const int DEFAULT_TTL = 1;
    ...
    public UDPMulticastProvider(LCM lcm, URLParser up)
    {
      ...
      Console.WriteLine("about to set sock.Ttl");
      sock.Ttl = (short) ttl;

      Console.WriteLine("UDPMulticastProvider inetAddr: " + inetAddr);
      sock.JoinMulticastGroup(inetAddr);
      Console.WriteLine("JoinMulticastGroup success");
    }

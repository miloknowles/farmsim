...
public class ObjectMesherViz : MonoBehaviour, LcmSubscriber
{
  ...
  public Color nearVertexColor = Color.cyan;
  public Color farVertexColor = Color.red;

  private Mesh mesh;
  private LcmHandle lcmHandle;
  private string meshChannel = "object_mesher/mesh";
  private Vector3[] newVertices;
  private float[] newRanges;
  ...
  void Update()
  {
    ...
    // Update mesh colors.
    Array.Resize(ref this.newUVs, this.newVertices.Length);

    for (int i = 0; i < this.newVertices.Length; ++i) {
      float range = this.newVertices[i].magnitude;
      float t = Mathf.Clamp(range / this.maxRange, 0.0f, 1.0f);
      this.newUVs[i].x = t;
      this.newUVs[i].y = 1.0f - t;
      // this.newColors[i] = Color.Lerp(this.nearVertexColor, this.farVertexColor, t);
    }

    this.mesh.Clear();
    this.mesh.SetVertices(this.newVertices);
    // this.mesh.SetColors(this.newColors);
    this.mesh.SetUVs(0, this.newUVs);
    this.mesh.SetTriangles(this.newTriangles, 0);
  ...
  public void MessageReceived(LcmHandle lcm, string channel, LcmStream dins)
  {
    if (channel == this.meshChannel) {
      ...
      // Array.Resize(ref this.newColors, msg.mesh.vertices.Length);
      Array.Resize(ref this.newUVs, msg.mesh.vertices.Length);
      // Array.Resize(ref this.newRanges, msg.mesh.vertices.Length);
